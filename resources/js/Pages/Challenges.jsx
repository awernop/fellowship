...
export default function Challenges({ posts }) {
        ...
        <AuthenticatedLayout>
            <Head title="Вызовы" />
                <div className="flex flex-col gap-3">
                    <div className="flex justify-between gap-3 w-full p-3 bg-gray-50 rounded-xl shadow-[1px_1px_16px_rgba(163,177,198,0.3),-8px_-8px_16px_rgba(255,255,255,0.3)]">
  ...
  <button
    ...
    className={`relative z-10 px-3 py-2 rounded-full transition-colors duration-200 ${
      displayFormat === 'cards' ? 'text-night' : 'text-gray-500 hover:text-gray-700'
    }`}
    title="Карточки"
  >
  ...
  <div 
    className={`absolute top-1 bottom-1 left-1 bg-white rounded-full shadow-sm transition-all duration-200 ${
      displayFormat === 'cards' ? 'translate-x-0 w-[49px]' : 'translate-x-[48px] w-[48px]'
    }`}
  />
  ...
  <button
    ...
    className={`relative z-10 px-3 py-2 rounded-full transition-colors duration-200 ${
      displayFormat === 'table' ? 'text-night' : 'text-gray-500 hover:text-gray-700'
    }`}
    title="Таблица"
  >
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
